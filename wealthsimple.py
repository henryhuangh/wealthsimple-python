...
class wealthsimple(object):
        def __init__(self, email, password, MFA=None):
                if MFA==None:
                        ...
                        r=requests.post('https://trade-service.wealthsimple.com/auth/login', data={'email':email, 'password':password, 'otp': MFA})
        ...
        def balance(self, account_id):
                try:
                        r=requests.get(self.url+'/account', params={'account_id':account_id}, headers={'authorization':self.access_token})
        ...
        def positions(self, account_id):
                try:
                        r=requests.get(self.url+'/account/positions', params={'account_id':account_id}, headers={'authorization':self.access_token})
        ...
        def activities(self, account_id, limit=20):
                try:
                        r=requests.get(self.url+'/account/activities', params={'account_id':account_id, 'limit':limit}, headers={'authorization':self.access_token})
                        return r.json()['results']
                except:
                        return False
        ...
        def order_history(self):#depreceated?
                try:
                        r=requests.get(self.url+'/orders', headers={'authorization':self.access_token})
                        return r.json()
                except:
                        return False

        def limit_buy(self, tick_id, quantity, price, account_id=None):#time_in_force options = "until_cancel", "day"
                try:
                        if account_id==None:
                                ...
                                r=requests.post(self.url+'/orders', json={'security_id':tick_id, 'limit_price':price, 'quantity': quantity, 'order_type': 'buy_quantity',  'order_sub_type': 'limit', 'account_id':account_id, 'time_in_force': 'day'}, headers={'authorization':self.access_token})
                                return r.json()['order_id']
        ...
        def stop_limit_buy(self, tick_id, quantity, price, stop_price, account_id=None):
                try:
                        if account_id==None:
                                r=requests.post(self.url+'/orders', json={'security_id':tick_id, 'limit_price':price, 'stop_price':stop_price, 'quantity': quantity, 'order_type': 'buy_quantity',  'order_sub_type': 'stop_limit', 'time_in_force': 'day'}, headers={'authorization':self.access_token})
                                return r.json()['order_id']
                        else:
                                r=requests.post(self.url+'/orders', json={'security_id':tick_id, 'limit_price':price, 'stop_price':stop_price, 'quantity': quantity, 'order_type': 'buy_quantity',  'order_sub_type': 'stop_limit', 'account_id':account_id, 'time_in_force': 'day'}, headers={'authorization':self.access_token})
                                return r.json()['order_id']
        ...
        def limit_sell(self, tick_id, quantity, price, account_id=None):
                try:
                        if account_id==None:
                                ...
                                r=requests.post(self.url+'/orders', json={'security_id':tick_id, 'limit_price':price, 'quantity': quantity, 'order_type': 'sell_quantity',  'order_sub_type': 'limit', 'account_id':account_id, 'time_in_force': 'day'}, headers={'authorization':self.access_token})
                                return r.json()['order_id']
                except:
                        return False
        ...
        def stop_limit_sell(self, tick_id, quantity, price, stop_price, account_id=None):
                try:
                        if account_id==None:
                                r=requests.post(self.url+'/orders', json={'security_id':tick_id, 'limit_price':price, 'stop_price':stop_price, 'quantity': quantity, 'order_type': 'sell_quantity',  'order_sub_type': 'stop_limit', 'time_in_force': 'day'}, headers={'authorization':self.access_token})
                                return r.json()['order_id']
                        else:
                                r=requests.post(self.url+'/orders', json={'security_id':tick_id, 'limit_price':price, 'stop_price':stop_price, 'quantity': quantity, 'order_type': 'sell_quantity', 'account_id':account_id,  'order_sub_type': 'stop_limit', 'time_in_force': 'day'}, headers={'authorization':self.access_token})
                                return r.json()['order_id']

                except:
                        return False
                
        def market_buy(self, tick_id, quantity, price=1, account_id=None):#all market buys must have limit price
                try:
                        if account_id==None:
                                ...
                                r=requests.post(self.url+'/orders', json={'security_id':tick_id, 'limit_price': price, 'quantity': quantity, 'order_type': 'buy_quantity', 'account_id':account_id,  'order_sub_type': 'market', 'time_in_force': 'day'}, headers={'authorization':self.access_token})
                                return r.json()['order_id']
                except:
                        return False

        def market_sell(self, tick_id, quantity, price=1, account_id=None):#all market sells must have limit price
                try:

                        if account_id==None:
                                r=requests.post(self.url+'/orders', json={'security_id':tick_id, 'market_value': price, 'quantity': quantity, 'order_type': 'sell_quantity', 'order_sub_type': 'market', 'time_in_force': 'day'}, headers={'authorization':self.access_token})
                                return r.json()['order_id']
                        else:
                                r=requests.post(self.url+'/orders', json={'security_id':tick_id, 'market_value': price, 'quantity': quantity, 'order_type': 'sell_quantity', 'account_id':account_id,  'order_sub_type': 'market', 'time_in_force': 'day'}, headers={'authorization':self.access_token})
                                return r.json()['order_id']
                except:
                        return False
        ...
        def get(self, endpoint, params='', json=''):
                ...
                r=requests.get(self.url+endpoint, params=params, json=json, headers={'authorization':self.access_token})
                return r


        def post(self, endpoint, params='', json=''):

                r=requests.post(self.url+endpoint, params=params, json=json, headers={'authorization':self.access_token})
                return r.json()


        def delete(self, endpoint, params='', json=''):

                r=requests.delete(self.url+endpoint, params=params, json=json, headers={'authorization':self.access_token})
                return r.json()
